...
SUDODEM_PLUGIN(
    ...
    (Superellipse)(SuperellipseGeom)(Bo1_Superellipse_Aabb)(SuperellipsePhys)(SuperellipseMat)(Ip2_SuperellipseMat_SuperellipseMat_SuperellipsePhys)(SuperellipseLaw)
...
/*sssssssssssssss*/
void Superellipse::Initial() {  // cout<<"watch init"<<init<<endl;
  if (init) return;
  //
  rx = rxy(0);
  ry = rxy(1);
  ...
  // The detailed derivations are shown in the reference, i.e., A. Jaklič, A.
  // Leonardis, F. Solina, Segmentation and Recovery of Superellipse, Springer
  ...
  Area = 2. * a * beta1;  // volume = 2a1a2a3ep1ep2B(ep1/2+1,ep1)B(ep2/2,ep2/2)
                          // see the reference
  ...
  // rotation matrices
  rot_mat2local =
      (Orientation).inverse().toRotationMatrix();     // to particle's system
  ...
  // test
  // Quaternionr
  // Rot(double(rand())/RAND_MAX,double(rand())/RAND_MAX,double(rand())/RAND_MAX,double(rand())/RAND_MAX);
  // Rot.normalize();
  // Orientation =Rot;
  // test_end
...
//****************************************************************************************
bool Superellipse::isInside(Vector2r p)  // check point p is inside the surface
                                         // using the inside-outside function
{
...
//****************************************************************************************
Vector2r Superellipse::getSurface(Real phi) const  // in local coordinates
                                                   // system
{
  Vector2r Surf;
...
Real Superellipse::Normal2Phi(Vector2r n) {  // n.normalize();
  ...
  {
          phi(1) = atan(Sign(n(2)) * pow(fabs(rz*n(2)*
  pow(fabs(cos(phi(0))),2.-eps1) / fabs(rx*n(0))), 1/(2.-eps2) ) );
  }
  else{
          phi(1) = atan(Sign(n(2)) * pow(fabs(rz*n(2)*
  pow(fabs(sin(phi(0))),2.-eps1) / fabs(ry*n(1))), 1/(2.-eps2) ) );
  }
  */
  // use atan2(y,x), which returns arctan(y/x).
  return atan2(
      Sign(n(1)) * pow((fabs(ry * n(1))), 1 / (2. - eps)),
      Sign(n(0)) * pow((fabs(rx * n(0))),
                       1 / (2. - eps)));  // atan2 returns in [-pi,pi]//atan(x)
                                          // returns in [-pi/2, pi/2]
}
/////////////////////////////////////////////////////////////////////////////////////////////////
...
{	Vector2r P;
        P(0) = Sign(cos(phi(0)))*rx*pow(fabs(cos(phi(0))),
eps1)*pow(fabs(cos(phi(1))), eps2); P(1) =
Sign(sin(phi(0)))*ry*pow(fabs(sin(phi(0))), eps1)*pow(fabs(cos(phi(1))), eps2);
        P(2) = Sign(sin(phi(1)))*rz*pow(fabs(sin(phi(1))), eps2);
        Mat32r p2phi;
        p2phi <<
                        P(0)*eps1*fabs(tan(phi(0))),
P(0)*eps2*fabs(tan(phi(1))), P(1)*eps1*fabs(cot(phi(0))),
P(1)*eps2*fabs(tan(phi(1))), P(2)*0.					  ,
P(2)*eps2*fabs(cot(phi(1)));
...
void Bo1_Superellipse_Aabb::go(const shared_ptr<Shape>& ig,
                               shared_ptr<Bound>& bv, const Se2r& se2,
                               const Body*) {

  Superellipse* t = static_cast<Superellipse*>(ig.get());
  if (!t->IsInitialized()) {
    t->Initial();
    // FIXME:Directly changing rotation in python (i.e., something like
    // O.bodies[i].state.ori = xxx) is not allowed temporarily, because this
    // action would not update rotation matrices:rot_mat2local and
    // rot_mat2global.
    t->rot_mat2local =
        (se2.rotation).inverse().toRotationMatrix();  // to particle's system
  ...
  // aabb->min=se2.position - halfsize*2;
  // aabb->max=se2.position + halfsize*2;
...
void Gl1_Superellipse::initSolidGlList(Superellipse* t) {
  ...
  // Generate the list. Only once for each qtView, or more if quality is
  // modified.
...
void Gl1_Superellipse::go(const shared_ptr<Shape>& cm,
                          const shared_ptr<State>& state, bool wire2,
                          const GLViewInfo&) {
  glMaterialv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE,
              Vector3r(cm->color[0], cm->color[1], cm->color[2]));
  glColor3v(cm->color);
  Superellipse* t = static_cast<Superellipse*>(cm.get());
  if (!t->IsInitialized()) {
    t->Initial();
    t->rot_mat2local =
        state->ori.inverse().toRotationMatrix();        // to particle's system
    t->rot_mat2global = state->ori.toRotationMatrix();  // to global system
  }

  bool somethingChanged =
      (t->m_GL_slices != Slices || glIsList(t->m_glSolidListNum) != GL_TRUE);
  if (somethingChanged) {
    // std::cout<<"haha preslices"<<t->m_GL_slices<<"slices"<<slices<<std::endl;
    initSolidGlList(t);
    initWireGlList(t);
    t->m_GL_slices = Slices;
  }
  if (wire || wire2) {
...
//
void Gl1_SuperellipseGeom::go(const shared_ptr<IGeom>& ig,
                              const shared_ptr<Interaction>&,
                              const shared_ptr<Body>&, const shared_ptr<Body>&,
                              bool) {
  ...
  point1 = (!scene->isPeriodic ? pg->point1
                               : scene->cell->wrapShearedPt(pg->point1));
  point2 = (!scene->isPeriodic ? pg->point2
                               : scene->cell->wrapShearedPt(pg->point2));
  // testing

  Vector2r point11, point22;
  point11 = (!scene->isPeriodic ? pg->point11
                                : scene->cell->wrapShearedPt(pg->point11));
  point22 = (!scene->isPeriodic ? pg->point22
                                : scene->cell->wrapShearedPt(pg->point22));
...
#endif
//**********************************************************************************
//! Precompute data needed for rotating tangent vectors attached to the
//! interaction

void SuperellipseGeom::precompute(const State& rbp1, const State& rbp2,
                                  const Scene* scene,
                                  const shared_ptr<Interaction>& c,
                                  const Vector2r& currentNormal, bool isNew,
                                  const Vector2r& shift) {
  if (!isNew) {
  ...
  Vector2r relativeVelocity = rbp2.vel - rbp1.vel -
                              rbp2.angVel * c2x.norm() * del -
                              rbp1.angVel * c1x.norm() * del;
  // Vector2r relativeVelocity = (rbp2.vel+rbp2.angVel.cross(c2x)) -
  // (rbp1.vel+rbp1.angVel.cross(c1x)); cout<<"relv1="<<relativeVelocity<<"
  // del="<<del<<endl;
  Vector2r shiftVel = scene->isPeriodic ? scene->cell->intrShiftVel(c->cellDist)
                                        : Vector2r::Zero();
  ...
  // cout<<"shiftVel="<<shiftVel<<" relv2="<<relativeVelocity<<"
  // revn="<<relativeVn<<" revs="<<relativeVs<<endl;
...
*/
  ...
  Vector2r preTangent = Vector2r(
      -preNormal[1], preNormal[0]);  // tangent is left-hand side of normal
...
void Ip2_SuperellipseMat_SuperellipseMat_SuperellipsePhys::go(
    const shared_ptr<Material>& b1, const shared_ptr<Material>& b2,
    const shared_ptr<Interaction>& interaction) {
  if (interaction->phys) return;
  const shared_ptr<SuperellipseMat>& mat1 =
      SUDODEM_PTR_CAST<SuperellipseMat>(b1);
  const shared_ptr<SuperellipseMat>& mat2 =
      SUDODEM_PTR_CAST<SuperellipseMat>(b2);
  interaction->phys = shared_ptr<SuperellipsePhys>(new SuperellipsePhys());
  const shared_ptr<SuperellipsePhys>& contactPhysics =
      SUDODEM_PTR_CAST<SuperellipsePhys>(interaction->phys);
...
//**************************************************************************************
#if 1
Real SuperellipseLaw::getPlasticDissipation() {
  return (Real)plasticDissipation;
}
void SuperellipseLaw::initPlasticDissipation(Real initVal) {
...
bool SuperellipseLaw::go(shared_ptr<IGeom>& ig, shared_ptr<IPhys>& ip,
                         Interaction* I) {

  if (!I->geom) {
    return true;
  }
  const shared_ptr<SuperellipseGeom>& contactGeom(
      SUDODEM_PTR_DYN_CAST<SuperellipseGeom>(I->geom));
  if (!contactGeom) {
    return true;
  }
  ...
  // Matrix2r cellHsize;
  // Vector2r ab_min, ab_max;
  // ab_min = B->bound->min;
  // ab_max = B->bound->max;
  // Vector2r shift2 = Vector2r(0.,0.);
  // if (scene->isPeriodic) {
  //   cellHsize = scene->cell->hSize;
  //   shift2 = cellHsize * I->cellDist.cast<Real>();

  //   ab_min += shift2;
  //   ab_max += shift2;
  // }

  // // erase the interaction when aAbB shows separation, otherwise keep it to
  // be
  // // able to store previous separating plane for fast detection of separation
  // if (A->bound->min[0] >= ab_max[0] || ab_min[0] >= A->bound->max[0] ||
  //     A->bound->min[1] >= ab_max[1] || ab_min[1] >= A->bound->max[1]) {
  //   phys->normalForce = Vector2r(0., 0.);
  //   phys->shearForce = Vector2r(0., 0.);
  //   scene->interactions->requestErase(I);
  //   if ((id1 == 19 && id2 == 399) || (id1 == 399 && id2 == 19)) {
  //     std::cout << "watch dog 1" << !I->geom << "\n";
  //   }
  //   return false;
  // }
  ...
  Vector2r normalForce =
      contactGeom->normal * contactGeom->PenetrationDepth * phys->kn;
  Vector2r shearForce1 =
      Vector2r::Zero();  // zhswee deprecated SuperellipseLaw.shearForce
  // shear force: in case the polyhdras are separated and come to contact again,
  // one should not use the previous shear force std::cerr <<
  // "p1:"<<gettid4()<<shearForce[0]<< "\n";
  ///////////////////////
  bool useDamping =
      (phys->betan > 1e-5 ||
       phys->betas > 1e-5);  // using viscous damping?
                             // tangential and normal stiffness coefficients,
                             // recomputed from betan,betas at every step
  Real cn = 0, cs = 0;
  ...
  if (useDamping) {  //
    ...
    Real mbar =
        (!A->isDynamic() && B->isDynamic())
            ? de2->mass
            : ((!B->isDynamic() && A->isDynamic())
                   ? de1->mass
                   : (de1->mass * de2->mass /
                      (de1->mass + de2->mass)));  // get equivalent mass if both
                                                  // bodies are dynamic, if not
                                                  // set it equal to the one of
                                                  // the dynamic body
    // Real mbar = de1->mass*de2->mass / (de1->mass + de2->mass); // equivalent
    // mass
    Real Cn_crit = 2. * sqrt(mbar * phys->kn);  // Critical damping coefficient
                                                // (normal direction)
    Real Cs_crit = 2. * sqrt(mbar * phys->ks);  // Critical damping coefficient
                                                // (shear direction)
    // Note: to compare with the analytical solution you provide cn and cs
    // directly (since here we used a different method to define c_crit)
    ...
    // this has been done when calculating the shear increamental displacement
    // (please see the function 'precompute')
    normalViscous = cn * contactGeom->relativeVn;
    // std::cerr << "normalViscous:"<<normalViscous(0)<<" "<<normalViscous(1)<<"
    // "<<normalViscous(2)<< "\n";
    Vector2r normTemp = normalForce - normalViscous;  // temporary normal force
    // viscous force should not exceed the value of current normal force, i.e.
    // no attraction force should be permitted if particles are non-adhesive
    // std::cerr << "nF1:"<<normalForce.norm()<< "\n";
    if (normTemp.dot(contactGeom->normal) < 0.0) {

      normalViscous = normalForce;  // normal viscous force is such that the
                                    // total applied force is null - it is
                                    // necessary to compute energy correctly!
      normalForce = Vector2r::Zero();
    } else {
      normalForce -= normalViscous;
    }
    // std::cerr << "nF2:"<<normalForce.norm()<< "\n";
  }
  // if(isnan(phys->shearForce[0])){std::cerr << "NAN,
  ...
  Real maxFs =
      normalForce.squaredNorm() * std::pow(phys->tangensOfFrictionAngle, 2);
  // PFC3d SlipModel, is using friction angle. CoulombCriterion
  if (shearForce1.squaredNorm() > maxFs) {
    Real ratio = sqrt(maxFs) / shearForce1.norm();
    shearForce1 *= ratio;
  } else {
    if (useDamping) {  // add current contact damping if we do not slide and if
                       // damping is requested
      shearViscous =
          cs *
          contactGeom->relativeVs;  // get shear viscous component
                                    // std::cerr << "sF1:"<<shearForce1(0)<<"
                                    // "<<shearForce1(1)<<" "<<shearForce1(2)<<
                                    // "\n"; std::cerr <<
                                    // "sF1:"<<shearForce1.norm()<< "\n";
  ...
  /*
          if (likely(!scene->trackEnergy  && !traceEnergy)){//Update force but
     don't compute energy terms (see below))
                  // PFC3d SlipModel, is using friction angle. CoulombCriterion
                  if( shearForce.squaredNorm() > maxFs ){
                          Real ratio = sqrt(maxFs) / shearForce.norm();
                          shearForce *= ratio;}
          } else {
                  //almost the same with additional Vector2r instatinated for
     energy tracing,
                  //duplicated block to make sure there is no cost for the
     instanciation of the vector when traceEnergy==false
                  if(shearForce.squaredNorm() > maxFs){
                          Real ratio = sqrt(maxFs) / shearForce.norm();
                          Vector2r trialForce=shearForce;//store prev force for
     definition of plastic slip
                          //define the plastic work input and increment the
     total plastic energy dissipated shearForce *= ratio; Real
     dissip=((1/phys->ks)*(trialForce-shearForce)).dot(shearForce); if
     (traceEnergy) plasticDissipation += dissip; else if(dissip>0)
  ...
  if (isnan(shearForce1[0]) || isnan(normalForce[0])) {
    ...
    // fprintf(fin,"normal_force\t%e\t%e\t%e\n",normalForce[0],normalForce[1],normalForce[2]);
    fprintf(fin, "shear_force\t%e\t%e\t%e\n", shearForce[0], shearForce[1],
            shearForce[2]);
    fprintf(fin, "shear_force1\t%e\t%e\t%e\n", shearForce1[0], shearForce1[1],
            shearForce1[2]);
    // fprintf(fin,"F\t%e\t%e\t%e\n",F[0],F[1],F[2]);
    fprintf(fin, "ks\t%emaxFs\t%esfn\t%e\n", phys->ks, maxFs,
            shearForce.squaredNorm());
    fprintf(fin, "shearDisp\t%e\t%e\t%e\n", shearDisp[0], shearDisp[1],
            shearDisp[2]);
    fprintf(fin, "normal_force\t%e\t%e\t%e\n", normalForce[0], normalForce[1],
            normalForce[2]);
    ...
    // scene->stopAtIter = scene->iter + 1;//Not fatal error for only one
    // iteration, so just pass it but with a warning.
    normalForce = Vector2r::Zero();
    shearForce1 = Vector2r::Zero();
  }
  Vector2r F = -normalForce - shearForce1 +
               shearViscous;  // the normal force acting on particle A (or 1) is
  ...
  // scene->forces.addTorque(idA,
  // -(A->state->pos-contactGeom->contactPoint).cross(F));
  // scene->forces.addTorque(idB,
  // (B->state->pos-contactGeom->contactPoint).cross(F)); needed to be able to
  // acces interaction forces in other parts of sudodem
