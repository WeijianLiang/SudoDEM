...
SUDODEM_PLUGIN(/* self-contained in hpp: */ (
    Ig2_Superellipse_Superellipse_SuperellipseGeom)(Ig2_Wall_Superellipse_SuperellipseGeom)(Ig2_Fwall_Superellipse_SuperellipseGeom)
...
void Disfunction(Real para, Vector2r& center, T* particle1, T* particle2,
  ...
  phi = particle1->Normal2Phi(contact1);  // caution: particle 1 (default) or
  ...
  // this case, the two particles are not touching. Thus the further detection is
  // not necessary.
...
bool Ig2_Superellipse_Superellipse_SuperellipseGeom::go(
  ...
  // particle's system A->rot_mat2global = (se21.orientation).toRotationMatrix();
    ...
    if (touching) {
      ...
      if (10.0 * (p2 - p1).norm() >
          A->getr_max()) {  // the contact depth is tremendously large which
                            ...
                            // particle size.FIXME:the coefficient of 10.0 may be
                            // not the best try.
        if (!(A->isInside(p2 - position1)) || !(B->isInside(p1 - position2))) {
          // FIXME:the inside-outside function may not be sufficiently accurate
          // for tiny contact depth during computation. cout<<"sure? not inside A
          // or B?"<<endl; cout<<"local minimia!"<<endl; cout<<"contact
...
bool Ig2_Wall_Superellipse_SuperellipseGeom::go(
  ...
  if (B->isSphere) {
    ...
    Matrix2r rot_mat1 = B->rot_mat2local;  //(se22.orientation).conjugate().toRotationMatrix();//to
                                           //particle's system
    Matrix2r rot_mat2 = B->rot_mat2global;  //(se22.orientation).toRotationMatrix();//to
                                            //global system
